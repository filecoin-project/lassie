...
func (e ConnectedToSPEvent) String() string {
	return fmt.Sprintf("ConnectedToSPEvent<%s, %s, %s>", e.eventTime, e.retrievalId, e.storageProviderId)
}

func Connected(at time.Time, retrievalId types.RetrievalID, candidate types.RetrievalCandidate) ConnectedToSPEvent {
	return ConnectedToSPEvent{spRetrievalEvent{retrievalEvent{at, retrievalId}, candidate.MinerPeer.ID}}
}
