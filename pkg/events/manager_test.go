...
func TestEventManager(t *testing.T) {
	...
	em.DispatchEvent(events.QueryAsked(id, queryStart, types.RetrievalCandidate{MinerPeer: peer.AddrInfo{ID: peerB}, RootCid: cid}, types.QueryResponse{Status: types.QueryResponseError, Size: 100, Message: "error @ response"}))
	...
	verifyQuerySuccess := func(event types.RetrievalEvent) {
		...
		require.Equal(t, types.QueryResponseError, event.(events.RetrievalEventQueryAsked).QueryResponse().Status)
