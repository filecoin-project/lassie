...
var (
	...
	_ EventWithEndpoint    = SucceededEvent{}
...
func (e SucceededEvent) String() string {
	return fmt.Sprintf("SucceededEvent<%s, %s, %s, %s, { %d, %d, %s, %s }>", e.eventTime, e.retrievalId, e.rootCid, e.endpoint, e.receivedBytesSize, e.receivedCidsCount, e.protocol, e.duration)
}
...
func Success(at time.Time, retrievalId types.RetrievalID, candidate types.RetrievalCandidate, receivedBytesSize uint64, receivedCidsCount uint64, duration time.Duration, protocol multicodec.Code) SucceededEvent {
	return SucceededEvent{providerRetrievalEvent{retrievalEvent{at, retrievalId, candidate.RootCid}, candidate.Endpoint()}, receivedBytesSize, receivedCidsCount, duration, protocol}
}
