...
func (e StartedRetrievalEvent) String() string {
	return fmt.Sprintf("StartedRetrievalEvent<%s, %s, %s, %s>", e.eventTime, e.retrievalId, e.storageProviderId, e.protocol)
}
...
func Started(at time.Time, retrievalId types.RetrievalID, candidate types.RetrievalCandidate, protocol multicodec.Code) StartedRetrievalEvent {
	return StartedRetrievalEvent{spRetrievalEvent{retrievalEvent{at, retrievalId}, candidate.MinerPeer.ID}, protocol}
}
