...
func TestRetrievalRacing(t *testing.T) {
	...
	testCases := []struct {
			...
			expectSequence: []testutil.ExpectedActionsAtTime{
				...
				},
				{
					...
					ExpectedEvents: []types.RetrievalEvent{
						...
						events.BlockReceived(startTime.Add(time.Millisecond*40+initialPause), retrievalID, types.RetrievalCandidate{MinerPeer: peer.AddrInfo{ID: peerFoo}}, multicodec.TransportGraphsyncFilecoinv1, 100),
			...
			expectSequence: []testutil.ExpectedActionsAtTime{
				...
				},
				{
					...
					ExpectedEvents: []types.RetrievalEvent{
						...
						events.BlockReceived(startTime.Add(time.Millisecond*520+initialPause), retrievalID, types.RetrievalCandidate{MinerPeer: peer.AddrInfo{ID: peerFoo}}, multicodec.TransportGraphsyncFilecoinv1, 100),
			...
			expectSequence: []testutil.ExpectedActionsAtTime{
				...
				},
				{
					...
					ExpectedEvents: []types.RetrievalEvent{
						...
						events.BlockReceived(startTime.Add(time.Millisecond*80), retrievalID, types.RetrievalCandidate{MinerPeer: peer.AddrInfo{ID: peerBar}}, multicodec.TransportGraphsyncFilecoinv1, 100),
			...
			expectSequence: []testutil.ExpectedActionsAtTime{
				...
				},
				{
					...
					ExpectedEvents: []types.RetrievalEvent{
						...
						events.BlockReceived(startTime.Add(time.Millisecond*221+initialPause), retrievalID, types.RetrievalCandidate{MinerPeer: peer.AddrInfo{ID: peerBaz}}, multicodec.TransportGraphsyncFilecoinv1, 100),
			...
			expectSequence: []testutil.ExpectedActionsAtTime{
				...
				},
				{
					...
					ExpectedEvents: []types.RetrievalEvent{
						...
						events.BlockReceived(startTime.Add(time.Millisecond*120+initialPause), retrievalID, types.RetrievalCandidate{MinerPeer: peer.AddrInfo{ID: peerBaz}}, multicodec.TransportGraphsyncFilecoinv1, 100),
			...
			expectSequence: []testutil.ExpectedActionsAtTime{
				...
				},
				{
					...
					ExpectedEvents: []types.RetrievalEvent{
						...
						events.BlockReceived(startTime.Add(time.Millisecond*421+initialPause), retrievalID, types.RetrievalCandidate{MinerPeer: peer.AddrInfo{ID: peerBang}}, multicodec.TransportGraphsyncFilecoinv1, 100),
		...
		{
			// Unlike the test with the same name against the full Retriever,
			// this test operates directly on the GraphsyncRetriever, so we can
			// tell it `noDirtyClose=true`, cancel the context, and check
			// whether it's still running or not. Proper completion of this test
			// indicates proper cleanup of per-SP goroutines.
			...
			retrievalReturns: map[string]testutil.DelayedClientReturn{
				"foo": {ResultStats: &types.RetrievalStats{StorageProviderId: peerFoo, Size: 1}, Delay: time.Second * 1},
				"bar": {ResultStats: &types.RetrievalStats{StorageProviderId: peerBar, Size: 2}, Delay: time.Second * 2},
				"baz": {ResultStats: &types.RetrievalStats{StorageProviderId: peerBaz, Size: 3}, Delay: time.Second * 3},
			},
			...
			expectSequence: []testutil.ExpectedActionsAtTime{
				...
				{
					AfterStart: time.Millisecond * 500,
					// cancellation here
				},
			},
		},
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			results := rv.RunWithVerification(
				ctx,
				t,
				clock,
				mockClient,
	...
	expectedSequence := []testutil.ExpectedActionsAtTime{
		...
		},
		{
			...
			ExpectedEvents: []types.RetrievalEvent{
				...
				events.BlockReceived(startTime.Add(time.Millisecond*300), retrievalID1, types.RetrievalCandidate{MinerPeer: peer.AddrInfo{ID: peerBar}}, multicodec.TransportGraphsyncFilecoinv1, 100),
		...
		},
		{
			...
			ExpectedEvents: []types.RetrievalEvent{
				...
				events.BlockReceived(startTime.Add(time.Millisecond*301+initialPause), retrievalID2, types.RetrievalCandidate{MinerPeer: peer.AddrInfo{ID: peerBing}}, multicodec.TransportGraphsyncFilecoinv1, 100),
	...
	expectedSequence := []testutil.ExpectedActionsAtTime{
		...
		},
		{
			...
			ExpectedEvents: []types.RetrievalEvent{
				...
				events.BlockReceived(startTime.Add(time.Millisecond*400+initialPause), retrievalID, types.NewRetrievalCandidate(peerBar, nil, cid.Undef, &metadata.GraphsyncFilecoinV1{PieceCID: cid.Cid{}, VerifiedDeal: true, FastRetrieval: false}), multicodec.TransportGraphsyncFilecoinv1, 100),
