			...
			expectedSequence: []testutil.ExpectedActionsAtTime{
				{
					...
					ExpectedEvents: []types.RetrievalEvent{
						...
						events.Started(startTime, rid, types.NewRetrievalCandidate(peerA, nil, cid1), multicodec.TransportGraphsyncFilecoinv1),
					},
				},
				{
					AfterStart: 20 * time.Millisecond,
					ExpectedEvents: []types.RetrievalEvent{
						events.Connected(startTime.Add(20*time.Millisecond), rid, types.NewRetrievalCandidate(peerA, nil, cid1)),
					},
			...
			expectedSequence: []testutil.ExpectedActionsAtTime{
				{
					...
					ExpectedEvents: []types.RetrievalEvent{
						...
						events.Started(startTime, rid, types.NewRetrievalCandidate(peerA, nil, cid1), multicodec.TransportGraphsyncFilecoinv1),
						events.Started(startTime, rid, types.NewRetrievalCandidate(peerB, nil, cid1), multicodec.TransportGraphsyncFilecoinv1),
					},
				},
				{
					AfterStart: 5 * time.Millisecond,
					ExpectedEvents: []types.RetrievalEvent{
						events.Connected(startTime.Add(5*time.Millisecond), rid, types.NewRetrievalCandidate(peerB, nil, cid1)),
					},
					ExpectedMetrics: []testutil.SessionMetric{
		...
		},
		{
			...
			expectedSequence: []testutil.ExpectedActionsAtTime{
				...
				},
				{
					...
					ExpectedEvents: []types.RetrievalEvent{
						events.Connected(startTime.Add(50*time.Millisecond), rid, types.NewRetrievalCandidate(peerA, nil, cid1)),
					},
					ExpectedMetrics: []testutil.SessionMetric{
...

		{
			...
			expectedSequence: []testutil.ExpectedActionsAtTime{
				...
				},
				{
					AfterStart: 50 * time.Millisecond,
					ExpectedEvents: []types.RetrievalEvent{
						events.Connected(startTime.Add(50*time.Millisecond), rid, types.NewRetrievalCandidate(peerB, nil, cid1)),
					},
					ExpectedMetrics: []testutil.SessionMetric{
...

		{
			...
			expectedSequence: []testutil.ExpectedActionsAtTime{
				...
				},
				{
					...
					ReceivedRetrievals: []peer.ID{peerA},
					ExpectedEvents: []types.RetrievalEvent{
						events.Connected(startTime.Add(500*time.Millisecond), rid, types.NewRetrievalCandidate(peerA, nil, cid1)),
					},
					ExpectedMetrics: []testutil.SessionMetric{
...

		{
			...
			expectedSequence: []testutil.ExpectedActionsAtTime{
				...
				},
				{
					...
					ExpectedEvents: []types.RetrievalEvent{
						events.Connected(startTime.Add(1*time.Millisecond), rid, types.NewRetrievalCandidate(peerA, nil, cid1)),
					},
					ExpectedMetrics: []testutil.SessionMetric{
				...
				},
				{
					...
					ExpectedEvents: []types.RetrievalEvent{
						events.Connected(startTime.Add(100*time.Millisecond), rid, types.NewRetrievalCandidate(peerB, nil, cid1)),
					},
					ExpectedMetrics: []testutil.SessionMetric{
	...
	results := testutil.RetrievalVerifier{
		ExpectedSequence: []testutil.ExpectedActionsAtTime{
			...
			},
			{
				AfterStart: 5 * time.Millisecond,
				ExpectedEvents: []types.RetrievalEvent{
					events.Connected(startTime.Add(5*time.Millisecond), rid, types.NewRetrievalCandidate(peerA, nil, cid1)),
				},
			},
			{
				AfterStart:         5*time.Millisecond + initialPause,
		...
		ExpectedSequence: []testutil.ExpectedActionsAtTime{
			{
				...
				ExpectedEvents: []types.RetrievalEvent{
					...
					events.Started(startTime.Add(10*time.Millisecond+initialPause), rid, types.NewRetrievalCandidate(peerA, nil, cid1), multicodec.TransportGraphsyncFilecoinv1),
					events.Started(startTime.Add(10*time.Millisecond+initialPause), rid, types.NewRetrievalCandidate(peerB, nil, cid1), multicodec.TransportGraphsyncFilecoinv1),
				},
			},
			{
				AfterStart: 5 * time.Millisecond,
				ExpectedEvents: []types.RetrievalEvent{
					events.Connected(startTime.Add(15*time.Millisecond+initialPause), rid, types.NewRetrievalCandidate(peerB, nil, cid1)),
				},
			},
			{
				AfterStart:         5*time.Millisecond + initialPause,
